...
from .models import Resource, Post, Attachment, Like, DisLike, Audience
...
def index(request):
    ...
    authors = User.objects.filter(pk__in=distinct_author_ids)
...
def dislike_post(request, post_id):
    ...
    # Check if the user has already liked the post
    if DisLike.objects.filter(user=User.objects.get(user=request.user), post=post).exists():
        # User has already liked the post, handle accordingly
